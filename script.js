...
const powerOn = () => {
  ...
  displayTimer = setTimeout(() => {
    setDisplay('Power On');
    displayTimer = setTimeout(() => setDisplay(`Kit : ${state.currentPadBankId}`), 1500);
  }, 500); 
...
const loadKit = (input) => {
  ...
  kits.forEach(i => i.nextElementSibling.classList.replace('is-on', 'is-off'));
...
document.addEventListener('keydown', (e) => {
  ...
  const padData = BANKS[state.currentPadBankId].find(p => p.keyTrigger === key);
  if (!padData) return;
